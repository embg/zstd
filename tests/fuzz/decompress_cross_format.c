 ...
 * Most importantly, we want to ensure that the magicless decoder does not reject
 * on valid frames.
 */
...
int LLVMFuzzerTestOneInput(const uint8_t *src, size_t size)
{
    ...
    const void* magiclessSrc = src;
    size_t const dstSize = FUZZ_dataProducer_uint32Range(producer, 0, 10 * size);
    void *standardDst = FUZZ_malloc(dstSize);
    void *magiclessDst = FUZZ_malloc(dstSize);
    ...
    // Test streaming decompression
    {
        ...
        if (standardRet == 0 && magiclessRet == 0) {
            ...
            FUZZ_ASSERT(
                memcmp(standardOut.dst, standardOut.dst, standardOut.pos) == 0
            );
        }
    }
